...
@section('content')
<h1 class="text-2xl font-bold mb-6 text-[color:var(--brand-maroon,#7b1d2e)]">
...
<form method="POST"
  ...
  <div>
    ...
    <select name="company_id"
            class="border rounded-lg px-3 py-2 w-full focus:ring-2 focus:ring-[color:var(--brand-maroon,#7b1d2e)] focus:outline-none">
      <option value="" {{ old('company_id', $site->company_id) ? '' : 'selected' }}>— (Tidak terkait perusahaan)</option>
      @foreach(($companies ?? []) as $c)
        <option value="{{ $c->id }}"
          @selected((string)old('company_id', $site->company_id) === (string)$c->id)>
          {{ $c->code }} — {{ $c->name }}
        </option>
      @endforeach
    </select>
    <p class="text-xs text-slate-500 mt-1">Opsional. Pilih perusahaan pemilik site.</p>
  </div>

  <div>
    <label class="block mb-1 font-semibold text-gray-700">
      Code <span class="text-sm text-gray-500">(mis: HO, BGG, SBS, DBK)</span>
    </label>
  ...
  <div class="flex gap-3">
    <button class="px-5 py-2.5 rounded-lg bg-[color:var(--brand-maroon,#7b1d2e)] hover:brightness-105 text-white shadow">
      Simpan
    </button>
