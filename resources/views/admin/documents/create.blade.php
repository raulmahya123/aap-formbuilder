        ...
        <div class="grid gap-3">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input name="project_code" class="mt-1 w-full border rounded-lg px-3 py-2" placeholder="CT-001 / PROJ-ABC">
            </div>
          </div>
  ...
  return {
    ...
    applyTemplate(tpl){
      ...
      if (!hasHeader) {
        ...
        this.preview.blocks.push({
          ...
          showMeta: false, metaRight: '', top: Math.max(8, (L.margins.top - 28)),
          left: L.margins.left, width: L.page.width - (L.margins.left + L.margins.right),
      ...
      if (!hasSig && Array.isArray(this.signatures?.rows) && this.signatures.rows.length) {
        ...
        this.preview.blocks.push({
          id: Math.random().toString(36).slice(2,10), type: 'signature',
          role: 'Disetujui oleh', name: this.signatures.rows?.[0]?.name || '', position: this.signatures.rows?.[0]?.position_title || '',
          top: y, left: L.margins.left, width: L.page.width - (L.margins.left + L.margins.right), height: h,
    ...
    rebuildRepeatingBlocksAcrossPages(){
      ...
      for (let pg = 2; pg <= pages; pg++){
        const clones = base.map(b => { const nb = JSON.parse(JSON.stringify(b)); nb.id = Math.random().toString(36).slice(2,10); nb.page = pg; return nb; });
    ...
    addPage(){
      ...
      const base = this.getRepeatingTemplateBlocks();
      const clones = base.map(b => { const nb = JSON.parse(JSON.stringify(b)); nb.id = Math.random().toString(36).slice(2,10); nb.page = newPage; return nb; });
